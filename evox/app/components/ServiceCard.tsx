...
export default function ServiceCard({ title, description, href, image }: ServiceCardProps) {
  return (
    <Link 
      href={href} 
      className="rounded-lg group relative flex items-end basis-89 w-full h-110 bg-black overflow-hidden pl-5 pt-5 pb-5 pr-30 
      before:content-[''] before:absolute before:bottom-0 before:left-0 before:w-full before:h-full before:z-[2] 
      before:inset-0 before:bg-gradient-to-t before:from-[#035f8a] before:to-[rgba(253,187,45,0)]"
    >
      <Image
      ...
      <div className="relative z-[2]">
        ...
        <p className="text-sm max-h-0 opacity-0 -translate-x-full transition-all duration-500 ease-in-out group-hover:max-h-40 group-hover:opacity-100 group-hover:translate-x-0">
          {description}
        </p>
      </div>
